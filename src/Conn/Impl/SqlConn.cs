...
{
    internal class SqlConn : AbstractConn, IConn
    {
        private SqlConnection _Instance;
        public SqlConnection Instance { get { return _Instance; } }

        public SqlConn(DCredential credential) : base(credential) { }
        ...
        protected override void SetCredential(DCredential credential)
        {
            string connStr = string.Format("Data Source={0};Initial Catalog={1};User ID={2};Password={3}"
                , credential.Host, credential.DBName, credential.Username, credential.Password);
            this.SetConnStr(connStr);
        }

        protected override void SetConnStr(string connStr)
        {
            this._Instance = new SqlConnection(connStr);
        }
