...
{
    internal class OdbcHandler : AbstractHandler, IHandler, IRDHandler
    {
        private OdbcConnection OdbcConn
        {
            get
            {
                var conn = base.Conn as OdbcConn;
                return conn != null ? conn.Instance : null;
            }
        }
        
        private OdbcDataAdapter Adapter { get; set; }

        private OdbcCommand Cmd { get; set; }

        public OdbcHandler(IConn conn) : base(conn) { }

        private OdbcParameter[] ToParamsArray(Param[] parameters)
        {
            if (parameters != null)
            {
                ...
                {
                    result[i] = new OdbcParameter(parameters[i].Name, parameters[i].Value);
                }
                return result;
            }
            return null;
        ...
        private OdbcCommand CreateCommand(string sql, Param[] parameters)
        {
            OdbcCommand cmd = new OdbcCommand(sql, this.OdbcConn);
        ...
        {
            OdbcCommand cmd = this.CreateCommand(sql, parameters);
            this.Adapter = new OdbcDataAdapter(cmd);
        ...
        {
            OdbcCommandBuilder builder = new OdbcCommandBuilder(this.Adapter);
        ...
        {
            this.Cmd = this.CreateCommand(sql, parameters);
            OdbcDataReader reader = this.Cmd.ExecuteReader();
        ...
        public void NoQueryHandleTran(params DCommand[] cmds)
        {
            ...
            OdbcTransaction tran = this.OdbcConn.BeginTransaction();
            try
            {
                this.NoQResult = 0;
