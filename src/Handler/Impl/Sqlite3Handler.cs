...
{
    internal class Sqlite3Handler : AbstractHandler, IHandler, IRDHandler
    {
        private SQLiteConnection SqliteConn
        {
            get
            {
                var conn = base.Conn as Sqlite3Conn;
        ...
        public Sqlite3Handler(IConn conn) : base(conn) { }

        private SQLiteParameter[] ToParamsArray(Param[] parameters)
        {
            if (parameters != null)
            {
                ...
                {
                    result[i] = new SQLiteParameter(parameters[i].Name, parameters[i].Value);
                }
                return result;
            }
            return null;
        ...
        private SQLiteCommand CreateCommand(string sql, Param[] parameters)
        {
            SQLiteCommand cmd = new SQLiteCommand(sql, this.SqliteConn);
        ...
        {
            SQLiteCommand cmd = this.CreateCommand(sql, parameters);
        ...
        public void NoQueryHandleTran(params DCommand[] cmds)
        {
            ...
            SQLiteTransaction tran = this.SqliteConn.BeginTransaction();
            try
            {
                this.NoQResult = 0;
